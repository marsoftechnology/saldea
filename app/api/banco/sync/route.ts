...
export async function POST(req: NextRequest) {
  ...
  // Solo Plan Max
  const adminCheck = createServiceRoleClient()
  const { data: cfgCheck } = await adminCheck.from('configuraciones_usuario').select('plan').eq('org_id', orgId).maybeSingle()
  if (cfgCheck?.plan !== 'max') {
