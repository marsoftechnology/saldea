...
interface EstadoQuipu {
  connected: boolean
  lastSync?: string | null
}

export default function QuipuSection() {
  const [estado, setEstado] = useState<EstadoQuipu | null>(null)
  ...
  async function cargar() {
    setCargando(true)
    try {
      const res = await fetch('/api/quipu/status')
      if (!res.ok) {
  ...
  async function conectar() {
    if (!token.trim()) {
      setAviso({ tipo: 'error', texto: 'Introduce tu token de acceso de Quipu' })
      return
    }
    setConectando(true)
    setAviso(null)
    try {
      const res = await fetch('/api/quipu/connect', {
        method: 'POST',
      ...
      const data = await res.json()
      if (!res.ok) {
        setAviso({ tipo: 'error', texto: data.error || 'Error al conectar con Quipu' })
        return
      }
      setAviso({ tipo: 'ok', texto: '✓ Quipu conectado. Ya puedes sincronizar tus facturas.' })
      setToken('')
      await cargar()
    } catch {
  ...
  async function sincronizar() {
    ...
    try {
      const res = await fetch('/api/quipu/sync', { method: 'POST' })
      const data = await res.json()
      if (!res.ok) {
        setAviso({ tipo: 'error', texto: data.error || 'Error al sincronizar con Quipu' })
        return
      }
  ...
  if (cargando) {
    return (
      <div className="mb-3 bg-zinc-900/40 border border-white/10 rounded-xl p-5">
        <p className="text-sm text-zinc-500">Cargando estado de Quipu…</p>
      </div>
    )
  }
  ...
  return (
    <div id="quipu" className="mb-3 bg-zinc-900/40 border border-white/10 rounded-xl overflow-hidden">
      <div className="p-5">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                <h2 className="text-base font-semibold text-zinc-100">Conectar con Quipu</h2>
                {conectado && (
              ...
              <p className="text-xs text-zinc-400 mt-1">
                {conectado
                  ? 'Importa facturas de Quipu para que Saldea persiga su cobro automáticamente.'
                  : 'Importa tus facturas pendientes de Quipu con un clic. Saldea se encarga del resto.'}
              </p>
            </div>
          </div>
        </div>
            ...
            >
              {sincronizando ? (
                <>
                  ...
                  Sincronizando con Quipu…
                </>
              ) : (
                <>🔄 Sincronizar facturas de Quipu</>
              )}
            </button>
          ...
          <div className="space-y-5">
            ...
            <div className="bg-zinc-800/50 border border-white/5 rounded-xl p-4">
              <p className="text-xs font-semibold text-zinc-300 mb-3 uppercase tracking-wide">
                ¿Cómo obtener tu token de Quipu?
              </p>
              <ol className="space-y-3 text-sm text-zinc-400">
                <li className="flex gap-3">
                  ...
                  <span>
                    ...
                    <a href="https://getquipu.com" target="_blank" rel="noopener noreferrer" className="text-sky-400 hover:underline">
                      getquipu.com
                    </a>
                  </span>
                </li>
                <li className="flex gap-3">
                  ...
                  <span>
                    Ve a <strong className="text-zinc-300">Configuración</strong> → <strong className="text-zinc-300">Desarrolladores</strong> → <strong className="text-zinc-300">Token de acceso</strong>
                  </span>
                </li>
                <li className="flex gap-3">
                  ...
                  <span>Copia el token y pégalo aquí abajo</span>
                </li>
              </ol>
            </div>
            ...
            <div>
              ...
              <div className="flex gap-2">
                <div className="relative flex-1">
                  <input
                    ...
                    placeholder="Pega aquí tu token de Quipu"
              ...
              <p className="text-xs text-zinc-600 mt-1.5">
                El token se guarda encriptado. Nunca lo compartiremos.
              </p>
            </div>
            ...
            <div className="border-t border-white/5 pt-4 text-xs text-zinc-500 space-y-1.5">
              ...
              <p>· Las facturas ya cobradas en Quipu no se importan</p>
