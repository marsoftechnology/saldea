        ...
        <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
          {nav.map(item => (
            <Link
              key={item.href}
              href={item.href}
              className={`flex items-center gap-3 px-3 py-2 rounded-lg text-sm border border-transparent transition-colors ${
                item.seccion
                  ? 'text-zinc-200 font-semibold hover:bg-white/[0.04]'
          ...
          <div className="pt-4 mt-4 border-t border-white/5">
            ...
            <div className="px-3 py-2 text-xs text-zinc-600 italic">Sin apps adicionales aún</div>
          </div>
        </nav>
